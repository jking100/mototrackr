...
const userSubmissionItemService = {
    ...
    create: async (userSubmissionItem) => {
        ...
        if(!response.ok) throw new Error("Failed to post: " + JSON.stringify(userSubmissionItem));
        return response.json();
    },
     ...
     */
    read: async (id) => {
        const response = await fetch(`${HTTPS_API_BASE_URL}/UserSubmissionItems/${id}`);
        if (!response.ok) throw new Error("Failed to get item by id: " + id.toString());
        return response.json();
    },
    ...
    update: async (id, updates) => {
        ...
        if(!response.ok) throw new Error(`Failed to update item ${id} with data: ${updates}`);
        return response.json();
    },
